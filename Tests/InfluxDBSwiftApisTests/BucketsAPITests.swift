...
class BucketsAPITests: APIXCTestCase {
    override func setUp() {
        super.setUp()
        api.bucketsAPI.getBuckets(limit: 100) { buckets, _ in
            buckets?
                    .buckets?
                    .filter { bucket in
                        bucket.name.hasSuffix("_TEST")
                    }.forEach { bucket in
                        self.api.bucketsAPI.deleteBucketsID(bucketID: bucket.id!) { _, _ in
                        }
                    }
        }
    }
    ...
    func testCreateBucket() {
        ...
        checkPost(api.bucketsAPI.postBuckets, request, &checker)
    }
}
